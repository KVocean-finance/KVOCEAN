...
function buildRequestedFormulaRows() {
  // 결과물(3.결과물 탭) 섹션·지표 출력 순서와 동일하게 정렬.
  // 핵심지표 → 비용구조 → 자산/부채 → 안정성 → 수익성 → 활동성 → 성장성.
  return [
    // 핵심 지표
    ...
    // 활동성 비율
    { 항목: "총자산회전율", 수식: "매출액 / ((전년도 4분기 자산 + 현재 분기 자산) / 2)" },
    { 항목: "매출채권회전율", 수식: "매출액 / ((전년도 4분기 매출채권 + 현재 분기 매출채권) / 2)" },
    { 항목: "매출채권회전기간", 수식: "365일 / 매출채권회전율" },
    { 항목: "재고자산회전율", 수식: "매출원가 / ((전년도 4분기 재고자산 + 현재 분기 재고자산) / 2)" },
